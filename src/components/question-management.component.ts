...
@Component({
  ...
  template: `
    <div class="container py-8">
      <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex gap-2">
          ...
          <div class="connection-status" 
            ...
            <span class="status-icon">{{ mongoConfig.isConnected ? '🟢' : '🔴' }}</span>
            <span class="status-text">
              {{ mongoConfig.isConnected ? 'MongoDB Connected' : 'MongoDB Disconnected' }}
            </span>
          </div>
      ...
      <div class="mb-6">
        <div class="flex gap-4 items-center flex-wrap">
          ...
          >
          <select [(ngModel)]="filterCategory" (ngModelChange)="filterQuestions()" class="select">
            <option value="">Tất cả danh mục</option>
            <option *ngFor="let category of categories" [value]="category">{{ category }}</option>
          </select>
          <select [(ngModel)]="filterDifficulty" (ngModelChange)="filterQuestions()" class="select">
      ...
      <div class="grid gap-4">
        <div *ngFor="let question of filteredQuestions; let i = index" class="card">
          <div class="flex justify-between items-start gap-4">
            <div class="flex-1">
              <h3 class="font-semibold mb-2">{{ i + 1 }}. {{ question.content }}</h3>
              ...
              <div class="flex gap-2 text-xs">
                <span class="px-2 py-1 bg-gray-100 rounded">{{ question.category || 'Chưa phân loại' }}</span>
                <span class="px-2 py-1 bg-gray-100 rounded">{{ getDifficultyText(question.difficulty) }}</span>
            ...
            <div class="flex gap-2">
              <button class="btn btn-outline btn-sm" (click)="editQuestion(question)">
                Sửa
              </button>
              <button class="btn btn-danger btn-sm" (click)="confirmDeleteQuestion(question.id)">
                Xóa
              </button>
            </div>
          </div>
      ...
      <div *ngIf="filteredQuestions.length === 0" class="text-center py-8 text-gray-500">
        Không tìm thấy câu hỏi nào
      </div>
    </div>

    <!-- Add/Edit Question Dialog -->
    <div *ngIf="showQuestionDialog" class="dialog-overlay" (click)="closeQuestionDialog()">
      <div class="dialog-content" (click)="$event.stopPropagation()">
        <div class="dialog-header">
          <h2 class="text-xl font-bold">
            {{ isEditMode ? 'Sửa câu hỏi' : 'Thêm câu hỏi mới' }}
          </h2>
          <button class="dialog-close" (click)="closeQuestionDialog()">×</button>
        </div>
        ...
        <div class="dialog-body">
          ...
          <div class="form-group">
            ...
            <div class="space-y-3">
              <div *ngFor="let option of questionForm.options; let i = index" class="option-row">
                <div class="option-header">
                  <input 
                    ...
                    class="radio-input"
                  >
                  <label class="option-label">Phương án {{ getChar(65 + i) }}</label>
                </div>
                <input 
                  type="text" 
    ...
    <!-- Import PDF Dialog -->
    <div *ngIf="showImportDialog" class="dialog-overlay" (click)="closeImportDialog()">
          ...
          <div *ngIf="importResults" class="import-results">
            ...
            <div class="results-summary">
              <div class="result-item success">
                ...
                <span>{{ importResults.questions.length }} câu hỏi được thêm</span>
              </div>
              <div *ngIf="importResults.duplicatesFound > 0" class="result-item warning">
                <span class="result-icon">⚠️</span>
                <span>{{ importResults.duplicatesFound }} câu hỏi trùng lặp (bỏ qua)</span>
              </div>
              <div *ngIf="importResults.errors.length > 0" class="result-item error">
                <span class="result-icon">❌</span>
...
export class QuestionManagementComponent implements OnInit {
  ...
  private updateCategories() {
    const categorySet = new Set(this.questions.map(q => q.category).filter(Boolean));
    this.categories = Array.from(categorySet) as string[];
  }

  filterQuestions() {
    this.filteredQuestions = this.questions.filter(question => {
      const matchesSearch = question.content.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
        question.options.some(option => option.toLowerCase().includes(this.searchTerm.toLowerCase()));
  ...
  async confirmSaveQuestion() {
    if (!this.questionForm.content.trim() || this.questionForm.options.some(opt => !opt.trim())) {
    ...
    const confirmed = await this.dialogService.confirm({
      ...
      message: this.isEditMode ? 'Bạn có muốn cập nhật câu hỏi này?' : 'Bạn có muốn thêm câu hỏi này?',
  ...
  saveQuestion() {
    if (this.isEditMode && this.editingQuestionId) {
      this.questionService.updateQuestion(this.editingQuestionId, this.questionForm);
    } else {
  ...
  async processPDF() {
    ...
    try {
      const result = await this.questionService.processPDFFile(this.selectedFile);
